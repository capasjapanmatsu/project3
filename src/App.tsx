    ...
    <HelmetProvider>
      ...
      <GoogleMapsProvider>
        <div className="min-h-screen bg-gray-50 flex flex-col">
            <SEO />
            <Navbar />
            {/* Fixed Navbar の下にスペーサーを入れて、最上部のバナーが隠れないようにする（さらに+16px） */}
            <div aria-hidden="true" style={{ height: 'calc(112px + env(safe-area-inset-top, 0px))' }} />
      ...
      {!showSplash && (
          <MaintenanceProvider>
            <Layout>
              ...
              <Route path="/admin/revenue/owner/:ownerId" element={
                <AdminRoute>
                  <Suspense fallback={<DashboardSkeleton />}>
                    <AdminOwnerPayoutDetail />
                  </Suspense>
                </AdminRoute>
              } />
              <Route path="/admin/revenue/park/:parkId" element={
                <AdminRoute>
                  <Suspense fallback={<DashboardSkeleton />}>
                    <AdminParkRevenueDetail />
                  </Suspense>
                </AdminRoute>
              } />
