  ...
  const createIdentityVerificationSession = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-identity-verification`, {
        ...
        body: JSON.stringify({
          returnUrl: `${window.location.origin}/park-registration-agreement`
        }),
      });
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        <div className="flex items-center justify-between">
          <div className="flex flex-col items-center">
            ...
            <span className="text-sm mt-2">契約同意</span>
          </div>
          <div className="flex-1 h-1 mx-4 bg-gray-300">
            <div className={`h-full ${isAgreementChecked ? 'bg-blue-600' : 'bg-gray-300'}`} style={{ width: isAgreementChecked ? '100%' : '0%' }}></div>
          </div>
          <div className="flex flex-col items-center">
            <div className={`w-10 h-10 ${isIdentityVerified ? 'bg-green-600' : isAgreementChecked ? 'bg-blue-600' : 'bg-gray-400'} text-white rounded-full flex items-center justify-center`}>
              <Shield className="w-5 h-5" />
            </div>
            <span className="text-sm mt-2">本人確認</span>
          </div>
          <div className="flex-1 h-1 mx-4 bg-gray-300">
            <div className={`h-full ${isIdentityVerified ? 'bg-green-600' : 'bg-gray-300'}`} style={{ width: isIdentityVerified ? '100%' : '0%' }}></div>
          </div>
          <div className="flex flex-col items-center">
            <div className={`w-10 h-10 ${isIdentityVerified ? 'bg-blue-600' : 'bg-gray-400'} text-white rounded-full flex items-center justify-center`}>
              <Building className="w-5 h-5" />
            </div>
            <span className="text-sm mt-2">施設登録</span>
          </div>
        </div>
      </div>
      ...
      {isIdentityVerified && (
        <div className="bg-green-100 border border-green-300 text-green-800 rounded-lg p-4 flex items-start">
          ...
          <div>
            <p className="font-medium">本人確認が完了しました</p>
            <p className="text-sm mt-1">ドッグラン施設の登録に進むことができます。</p>
          </div>
        </div>
      )}
      ...
      {!isIdentityVerified ? (
          ...
          {!verificationSessionUrl ? (
            ...
            <Card className="p-6">
              <div className="text-center">
                ...
                <h2 className="text-xl font-semibold text-gray-900 mb-2">本人確認</h2>
                ...
                {isCreatingVerification ? (
                  <div className="flex flex-col items-center justify-center py-4">
                    ...
                    <p className="text-gray-600">本人確認セッションを準備中...</p>
                  </div>
                ) : (
                  <Button
        ...
        <Card className="p-6">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-900 mb-2">契約同意と本人確認が完了しました</h2>
            <p className="text-gray-600 mb-6">
              おめでとうございます！ドッグランオーナー契約への同意と本人確認が完了しました。
              次はドッグラン施設の登録に進みましょう。
            </p>
