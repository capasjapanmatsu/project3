-- クーポンテーブル完全修正SQL（facility_idエラー対応）

-- ==========================================
-- ステップ1: 既存のテーブルとポリシーを完全削除
-- ==========================================

-- 既存のポリシーを削除
DROP POLICY IF EXISTS "Facility owners can manage their coupons" ON facility_coupons;
DROP POLICY IF EXISTS "Everyone can view active coupons" ON facility_coupons;
DROP POLICY IF EXISTS "Users can manage their own coupons" ON user_coupons;
DROP POLICY IF EXISTS "Facility owners can view their coupon usage" ON user_coupons;

-- 既存のRPC関数を削除
DROP FUNCTION IF EXISTS get_coupon_stats(UUID);
DROP FUNCTION IF EXISTS obtain_coupon(UUID, UUID);
DROP FUNCTION IF EXISTS use_coupon(VARCHAR(255));

-- 既存のテーブルを削除
DROP TABLE IF EXISTS user_coupons CASCADE;
DROP TABLE IF EXISTS facility_coupons CASCADE;

-- ==========================================
-- ステップ2: 新しいテーブルを正しい構造で作成
-- ==========================================

-- 1. 施設クーポンテーブル
CREATE TABLE facility_coupons (
...
CREATE INDEX idx_facility_coupons_facility_id ON facility_coupons(facility_id);
CREATE INDEX idx_facility_coupons_active ON facility_coupons(is_active, start_date, end_date);
CREATE INDEX idx_user_coupons_user_id ON user_coupons(user_id);
CREATE INDEX idx_user_coupons_coupon_id ON user_coupons(coupon_id);
CREATE INDEX idx_user_coupons_facility_id ON user_coupons(facility_id);
CREATE INDEX idx_user_coupons_used ON user_coupons(is_used);

-- ==========================================
-- ステップ4: RLS (Row Level Security) ポリシー設定
-- ==========================================
...
BEGIN
    ...
    -- クーポン取得レコード作成（facility_idも追加）
    INSERT INTO user_coupons (user_id, coupon_id, facility_id, qr_code_token)
    VALUES (p_user_id, p_coupon_id, coupon_record.facility_id, qr_token);
...
-- 作成されたテーブルを確認
SELECT 'facility_coupons テーブル作成完了' as message
UNION ALL
SELECT 'user_coupons テーブル作成完了' as message
UNION ALL
SELECT 'インデックス作成完了' as message
UNION ALL  
SELECT 'RLSポリシー設定完了' as message
UNION ALL
SELECT 'RPC関数作成完了' as message; 