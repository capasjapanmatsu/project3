...
BEGIN
    ...
    SELECT id INTO suzuki_id FROM profiles WHERE email = 'suzuki.misaki@example.com';
    
    -- 友達リクエストの作成
    IF admin_id IS NOT NULL THEN
        
        -- 1. 田中さんから管理者へのフレンドリクエスト（保留中）
        IF tanaka_id IS NOT NULL THEN
            INSERT INTO friend_requests (sender_id, receiver_id, status, created_at) VALUES
            (tanaka_id, admin_id, 'pending', NOW() - INTERVAL '3 hours')
        ...
        IF sato_id IS NOT NULL THEN
            ...
            (admin_id, sato_id, 'accepted', NOW() - INTERVAL '2 days', NOW() - INTERVAL '1 day')
            ON CONFLICT (sender_id, receiver_id) DO UPDATE SET 
                status = EXCLUDED.status,
            ...
            (admin_id, sato_id, NOW() - INTERVAL '1 day'),
            (sato_id, admin_id, NOW() - INTERVAL '1 day')
        ...
        IF yamada_id IS NOT NULL THEN
            INSERT INTO friend_requests (sender_id, receiver_id, status, created_at, updated_at) VALUES
            (yamada_id, admin_id, 'accepted', NOW() - INTERVAL '4 days', NOW() - INTERVAL '3 days')
            ON CONFLICT (sender_id, receiver_id) DO UPDATE SET 
                status = EXCLUDED.status,
                updated_at = EXCLUDED.updated_at;
            ...
            -- 友達関係を作成（双方向）
            INSERT INTO friendships (user_id, friend_id, created_at) VALUES
            (admin_id, yamada_id, NOW() - INTERVAL '3 days'),
            (yamada_id, admin_id, NOW() - INTERVAL '3 days')
            ON CONFLICT (user_id, friend_id) DO NOTHING;
        END IF;
        ...
        IF suzuki_id IS NOT NULL THEN
            INSERT INTO friend_requests (sender_id, receiver_id, status, created_at, updated_at) VALUES
            (suzuki_id, admin_id, 'rejected', NOW() - INTERVAL '1 week', NOW() - INTERVAL '6 days')
            ON CONFLICT (sender_id, receiver_id) DO UPDATE SET 
                status = EXCLUDED.status,
                updated_at = EXCLUDED.updated_at;
        END IF;
...
WHERE p1.email = 'capasjapan@gmail.com' OR p2.email = 'capasjapan@gmail.com'
