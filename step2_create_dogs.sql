...
BEGIN
    ...
    IF admin_id IS NOT NULL THEN
        -- 既存の犬がいるかチェック
        SELECT COUNT(*) INTO existing_dog_count FROM dogs WHERE owner_id = admin_id;
        
        IF existing_dog_count = 0 THEN
            -- 管理者の犬を作成
            INSERT INTO dogs (owner_id, name, breed, age, weight, gender, personality, created_at) VALUES
            (admin_id, 'アッシュ', '雑種', 4, 15.0, 'オス', '賢くて人懐っこい性格です。他の犬とも仲良くできます。', NOW() - INTERVAL '1 year');
            
            RAISE NOTICE '管理者の犬「アッシュ」を作成しました';
        ELSE
            RAISE NOTICE '管理者にはすでに % 匹の犬が登録されています', existing_dog_count;
        END IF;
    ELSE
...
INSERT INTO dogs (owner_id, name, breed, age, weight, gender, personality, created_at) 
SELECT 
    p.id as owner_id,
    CASE 
...
FROM profiles p
WHERE p.email IN ('tanaka.taro@example.com', 'sato.hanako@example.com', 'yamada.jiro@example.com', 'suzuki.misaki@example.com')
...
WHERE p.email IN ('capasjapan@gmail.com', 'tanaka.taro@example.com', 'sato.hanako@example.com', 'yamada.jiro@example.com', 'suzuki.misaki@example.com')
