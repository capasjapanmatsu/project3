...
FROM dogs
WHERE owner_id = tanaka_id
LIMIT 1;
SELECT id INTO sato_dog_id
FROM dogs
WHERE owner_id = sato_id
LIMIT 1;
SELECT id INTO yamada_dog_id
FROM dogs
WHERE owner_id = yamada_id
LIMIT 1;
IF admin_id IS NOT NULL
AND admin_dog_id IS NOT NULL THEN -- 1. 管理者の犬と佐藤さんの犬の出会い（友達同士）
IF sato_id IS NOT NULL
AND sato_dog_id IS NOT NULL THEN
...
VALUES (
        ...
        sato_dog_id,
        '中央公園ドッグラン',
        'アッシュとハナちゃんがとても仲良く遊んでいました。ボール遊びを一緒に楽しんでいました🎾',
    ...
    (
        ...
        sato_dog_id,
        '代々木公園',
        '散歩中に偶然会いました。お互いすぐに覚えていて、尻尾を振って挨拶していました',
        NOW() - INTERVAL '3 days'
    ),
    (
        admin_id,
        ...
        sato_dog_id,
        '井の頭公園',
        '桜が綺麗でした。アッシュとハナちゃんが池の周りを一緒に散歩しました',
        NOW() - INTERVAL '1 week'
    ) ON CONFLICT (user1_id, user2_id, created_at) DO NOTHING;
END IF;
-- 2. 管理者の犬と山田さんの犬の出会い（友達同士）
IF yamada_id IS NOT NULL
AND yamada_dog_id IS NOT NULL THEN
INSERT INTO encounters (
        user1_id,
        user2_id,
        dog1_id,
...
VALUES (
        ...
        yamada_dog_id,
        '渋谷ドッグラン',
        'チョコちゃんとアッシュが初対面でしたが、すぐに仲良くなりました。チョコちゃんがとても優しい性格で、アッシュも安心していました',
        NOW() - INTERVAL '2 days'
    ),
    (
        admin_id,
        ...
        yamada_dog_id,
        '駒沢公園',
        'アジリティコースで一緒に遊びました。チョコちゃんがアッシュに遊び方を教えてくれていました',
        NOW() - INTERVAL '5 days'
    ),
    (
        admin_id,
        ...
        yamada_dog_id,
        '上野公園',
        '広い芝生でフリスビーをしました。チョコちゃんの運動能力に驚きました！',
        NOW() - INTERVAL '1 week'
    ) ON CONFLICT (user1_id, user2_id, created_at) DO NOTHING;
END IF;
-- 3. 管理者の犬と田中さんの犬の出会い（まだ友達ではないが出会いはある）
IF tanaka_id IS NOT NULL
AND tanaka_dog_id IS NOT NULL THEN
INSERT INTO encounters (
        user1_id,
        user2_id,
        dog1_id,
...
VALUES (
        ...
        tanaka_dog_id,
        '荒川河川敷',
        'ポチくんとアッシュが朝の散歩で会いました。ポチくんはとても人懐っこくて、すぐにアッシュと仲良くなりました',
    ...
    (
        ...
        tanaka_dog_id,
        '世田谷公園',
        '偶然同じ時間に散歩していて出会いました。ポチくんの元気な様子にアッシュも刺激を受けていました',
        NOW() - INTERVAL '2 days'
    ) ON CONFLICT (user1_id, user2_id, created_at) DO NOTHING;
END IF;
RAISE NOTICE '管理者アカウントの出会い記録を作成しました';
ELSE RAISE NOTICE '管理者アカウントまたは犬のデータが見つかりません';
END IF;
END $$;
-- ステップ12: 最終的な確認クエリ
...
SELECT p.name as met_user,
    ...
    CASE
        WHEN f.user_id IS NOT NULL THEN '友達'
        ELSE '未友達'
