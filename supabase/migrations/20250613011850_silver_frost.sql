/*
  ...
  3. Functions
    ...
    - `reorder_product_images` - Reorder product images
    - `delete_product_image` - Delete a product image
    - `set_main_product_image` - Set a product image as main
*/
...
-- Create policy for admin access (check if it exists first)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create policy for all users to view product images (check if it exists first)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
