-- Finalize cancel_order ambiguity fix keeping original arg names for client compatibility
...
declare
  ...
  p_order_id uuid := order_id; -- disambiguate
begin
...
declare
  v_order public.orders;
  v_item record;
  p_order_id uuid := order_id; -- shadow-safe local vars
...
begin
  ...
  if not public.can_cancel_order(p_order_id) then
