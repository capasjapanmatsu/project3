...
-- インデックスの作成（IF NOT EXISTS付き）
CREATE INDEX IF NOT EXISTS idx_access_logs_user_id ON access_logs(user_id);
CREATE INDEX IF NOT EXISTS idx_access_logs_lock_id ON access_logs(lock_id);
CREATE INDEX IF NOT EXISTS idx_access_logs_pin ON access_logs(pin);
CREATE INDEX IF NOT EXISTS idx_access_logs_status ON access_logs(status);
CREATE INDEX IF NOT EXISTS idx_access_logs_issued_at ON access_logs(issued_at DESC);
...
-- インデックスの作成（IF NOT EXISTS付き）
CREATE INDEX IF NOT EXISTS idx_dog_run_locks_lock_id ON dog_run_locks(lock_id);
CREATE INDEX IF NOT EXISTS idx_dog_run_locks_park_id ON dog_run_locks(park_id);
...
DROP POLICY IF EXISTS "Users can create own access logs" ON access_logs;
DROP POLICY IF EXISTS "Service role can update all logs" ON access_logs;
...
DROP POLICY IF EXISTS "Authenticated users can view locks" ON dog_run_locks;
DROP POLICY IF EXISTS "Admins can manage locks" ON dog_run_locks;
...
DROP TRIGGER IF EXISTS update_dog_run_locks_updated_at ON dog_run_locks;
