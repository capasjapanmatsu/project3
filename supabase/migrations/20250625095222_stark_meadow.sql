/*
# Smart Lock PIN System

1. New Tables
  - `smart_lock_pins` - Stores temporary PIN codes for smart locks
  - Updates to `user_entry_exit_logs` - For tracking entry/exit events
  - Updates to `user_entry_status` - For tracking current user status

2. Security
  - RLS policies for PIN access and logs
  - Functions for PIN generation and verification
  - Cleanup trigger for expired PINs

3. Changes
  - Added PIN-related fields to smart_locks table
  - Added functions for access control and occupancy tracking
*/
