/*
# Smart Lock PIN System Implementation

1. New Tables
  - `smart_lock_pins` - Stores temporary PIN codes for entry/exit
  - `user_entry_exit_logs` - Logs all entry and exit events

2. Changes
  - Added PIN-related columns to `smart_locks` table
  - Added exit tracking to `user_entry_status` table
  
3. Functions
  - Added PIN generation and verification functions
  - Added park occupancy calculation function
*/
...
-- Create policies for smart_lock_pins
...
-- Create policies for user_entry_exit_logs
...
-- Trigger to clean up expired PINs
