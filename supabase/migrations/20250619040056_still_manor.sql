...
1. New Columns
   - `orders.cancellable_until` - Timestamp when order can no longer be cancelled

2. New Functions
   - `can_cancel_order` - Checks if an order can be cancelled
   - `cancel_order` - Cancels an order and restores stock
   - `set_order_cancellable_until` - Sets cancellation window on new orders

3. Triggers
   - `set_order_cancellable_until_trigger` - Automatically sets cancellation window
*/
