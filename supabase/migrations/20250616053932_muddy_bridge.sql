...
〒861-0563 熊本県山鹿市鹿央町千田１７１８－１３
  ...
  -- Create the table with a unique constraint on template_type
  CREATE TABLE IF NOT EXISTS public.email_template_config (
    ...
    CONSTRAINT email_template_config_template_type_key UNIQUE (template_type)
  );

  -- Check if the magic_link template already exists
  IF EXISTS (SELECT 1 FROM public.email_template_config WHERE template_type = 'magic_link') THEN
    -- Update existing record
    UPDATE public.email_template_config
    SET 
      subject = 'ドッグパークJP - ログインリンク',
