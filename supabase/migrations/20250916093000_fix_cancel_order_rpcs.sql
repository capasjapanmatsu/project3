-- Fix: missing FROM-clause entry for table "cancel_order"
-- Root cause: SQL referred to cancel_order.order_id etc. in a function
-- whose name is the same as the table alias. Use function argument names directly.

create or replace function public.can_cancel_order(order_id uuid)
returns boolean
language plpgsql security definer set search_path = public as $$
declare v_order public.orders; begin
  select * into v_order from public.orders where id = can_cancel_order.order_id;
...
declare v_order public.orders; v_item record; begin
  select * into v_order from public.orders where id = cancel_order.order_id;
  if not found then
    return jsonb_build_object('success', false, 'message', 'Order not found');
  end if;
  if v_order.user_id <> cancel_order.user_id then
    return jsonb_build_object('success', false, 'message', 'You do not have permission to cancel this order');
  end if;
  if not can_cancel_order(cancel_order.order_id) then
    return jsonb_build_object('success', false, 'message', 'This order cannot be cancelled');
  end if;
  for v_item in select * from public.order_items where order_id = cancel_order.order_id loop
