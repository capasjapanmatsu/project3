/*
  ...
  1. New Functions
    - `cleanup_expired_qr_codes`: Updates expired QR codes and shares to 'expired' status
    - `cleanup_expired_trusted_devices`: Removes expired trusted devices
    - `cleanup_expired_2fa_codes`: Removes expired 2FA codes
    - `cleanup_old_error_logs`: Removes old error logs
    - `cleanup_old_auth_logs`: Removes old authentication logs
    - `cleanup_old_notifications`: Removes old read notifications
    - `get_database_size_info`: Returns size information for all tables
    - `get_index_usage_stats`: Returns index usage statistics
    - `get_table_bloat_info`: Returns table bloat information
    - `get_slow_queries`: Returns slow queries from pg_stat_statements
    - `analyze_all_tables`: Analyzes all tables
    - `vacuum_all_tables`: Vacuums all tables
    - `reindex_all_tables`: Reindexes all tables
    - `refresh_all_materialized_views`: Refreshes all materialized views
*/
