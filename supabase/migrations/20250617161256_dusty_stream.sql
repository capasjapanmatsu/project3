...
-- Create policies for the table (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_entry_status' 
    AND policyname = 'Users can view their own entry status'
  ) THEN
...
-- Create a trigger to run the cleanup function periodically (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'cleanup_expired_qr_codes_trigger'
  ) THEN
