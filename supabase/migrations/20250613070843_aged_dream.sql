/*
  ...
  3. Security
    - Enable RLS
    - Policies for admin management and user viewing
  
  4. Functions
    - Functions to get, reorder, delete, and set main product images
*/
...
BEGIN
  ...
  IF EXISTS (
    ...
    DROP POLICY "Only admins can manage product images" ON product_images;
  END IF;
  ...
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'product_images' 
    AND policyname = 'All users can view product images'
  ) THEN
    DROP POLICY "All users can view product images" ON product_images;
  END IF;
END
$$;
