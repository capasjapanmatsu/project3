/*
  # Fix user park access function and friend request policies

  1. Updates
    - Update `check_user_park_access` function to support shared QR codes and park owners
    - Add `can_send_friend_request` function to check if users have met at a dog park
  
  2. Policy Changes
    - Drop and recreate friend request policy with proper checks
*/

-- First drop the existing function
DROP FUNCTION IF EXISTS check_user_park_access(uuid, text);

-- Re-create the function with updated logic to support shared QR codes and park owners
...
BEGIN
  SELECT EXISTS (
    ...
    AND policyname = '友達申請は出会いがある場合のみ可能'
  ) INTO policy_exists;
  
  IF policy_exists THEN
    EXECUTE 'DROP POLICY "友達申請は出会いがある場合のみ可能" ON friend_requests';
  END IF;
END $$;
