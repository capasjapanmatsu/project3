/*
  ...
  1. New Data
    - 同じドッグランで同じ日に予約した犬の出会いデータ
    - 複数の犬と飼い主のサンプルデータ
    - 実際の犬の画像URL付き
  
  2. Security
    - 関数はSECURITY DEFINERで実行
*/

-- サンプルデータ生成関数
CREATE OR REPLACE FUNCTION create_dog_encounters_samples()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  dog4_id uuid;
  -- 予約日
  encounter_date date := current_date - interval '2 days';
BEGIN
  -- 現在のユーザーIDを取得
  ...
  IF current_user_id IS NOT NULL THEN
    ...
    VALUES
      (current_user_id, current_user_dog_id, sample_park_id, encounter_date, '13:00', 2, 'confirmed', 800, 'regular'),
      (user1_id, dog1_id, sample_park_id, encounter_date, '13:00', 2, 'confirmed', 800, 'regular'),
      (user2_id, dog2_id, sample_park_id, encounter_date, '14:00', 2, 'confirmed', 800, 'regular'),
      (user3_id, dog3_id, sample_park_id, encounter_date, '15:00', 2, 'confirmed', 800, 'regular'),
      (user4_id, dog4_id, sample_park_id, encounter_date, '13:30', 2, 'confirmed', 800, 'regular')
    ON CONFLICT DO NOTHING;
    
    -- 犬の出会い記録を作成（同じ日に同じドッグパークを利用した犬同士）
    INSERT INTO dog_encounters (dog1_id, dog2_id, park_id, encounter_date, created_at)
    VALUES
      (LEAST(current_user_dog_id, dog1_id), GREATEST(current_user_dog_id, dog1_id), sample_park_id, encounter_date, now() - interval '2 days'),
      (LEAST(current_user_dog_id, dog2_id), GREATEST(current_user_dog_id, dog2_id), sample_park_id, encounter_date, now() - interval '2 days'),
      (LEAST(current_user_dog_id, dog4_id), GREATEST(current_user_dog_id, dog4_id), sample_park_id, encounter_date, now() - interval '2 days'),
      (LEAST(dog1_id, dog2_id), GREATEST(dog1_id, dog2_id), sample_park_id, encounter_date, now() - interval '2 days'),
      (LEAST(dog1_id, dog4_id), GREATEST(dog1_id, dog4_id), sample_park_id, encounter_date, now() - interval '2 days')
    ON CONFLICT DO NOTHING;
...
-- サンプルデータ生成関数を実行
SELECT create_dog_encounters_samples();

-- 関数を削除
DROP FUNCTION create_dog_encounters_samples();