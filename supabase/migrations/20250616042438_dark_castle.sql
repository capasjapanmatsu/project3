/*
  # User Entry Status Tracking System

  1. New Tables
    - `user_entry_status` - Tracks which users are currently inside which parks
  
  2. Functions
    - `record_user_entry` - Records when a user enters a park
    - `record_user_exit` - Records when a user exits a park
    - `generate_exit_qr` - Generates QR code for exiting a park
    - `is_user_inside_park` - Checks if a user is currently inside a park
    - Updates to `generate_entrance_qr` function
  
  3. Triggers
    - Cleanup trigger for expired QR codes
*/
...
-- First drop the existing function to avoid parameter default issues
DROP FUNCTION IF EXISTS generate_entrance_qr(uuid, uuid[], text, integer);

-- Recreate the function with the updated logic
...
DROP FUNCTION IF EXISTS cleanup_expired_qr_codes() CASCADE;
